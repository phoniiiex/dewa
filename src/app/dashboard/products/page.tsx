...
const stockLabel = (p: Product) => p.stock <= p.lowStock ? "کەمە" : p.stock <= p.lowStock * 3 ? "کەم" : "بەردەست";
const stockColor = (p: Product) => p.stock <= p.lowStock ? "#FA5252" : p.stock <= p.lowStock * 3 ? "#FD7E14" : "#40C057";
            ...
            <div style={{ display: "flex", gap: 16, marginBottom: 10 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <span style={{ fontSize: 15, fontWeight: 800, color: stockColor(p) }}>{p.stock.toLocaleString()}</span>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={p.id}>
                  ...
                  <td>
                    <span style={{ fontWeight: 600, color: stockColor(p) }}>{p.stock.toLocaleString()}</span>{" "}
                    <span style={{ fontSize: 10, color: "#ADB5BD" }}>{p.unitType}</span>
                  </td>
                  <td><span style={{ padding: "2px 7px", borderRadius: 4, fontSize: 10, fontWeight: 600, background: stockColor(p) + "15", color: stockColor(p) }}>{stockLabel(p)}</span></td>
